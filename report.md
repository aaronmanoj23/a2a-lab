...
**Deployed Engine ID:** `6643856735196938240`  
**Full resource name:** `projects/376571806846/locations/us-central1/reasoningEngines/6643856735196938240`
...
The Vertex AI Agent Engine runtime calls `query()` **synchronously** — it is not an async-aware framework. The wrapper therefore implements `query()` as a plain synchronous method and handles the echo and summarise logic directly inline, without delegating to the async `handle_task` handler. This avoids the need for `asyncio.run()` entirely and keeps the wrapper self-contained and compatible with the Agent Engine runtime.
...
A2A Demo -- Echo Agent
============================================================
[A2AClient] GET https://echo-a2a-agent-tyvibunxva-uc.a.run.app/.well-known/agent.json
...
--- Echo Task ---
[A2AClient] POST https://echo-a2a-agent-tyvibunxva-uc.a.run.app/tasks/send
...
*Note: Output captured from the live Cloud Run deployment (`https://echo-a2a-agent-tyvibunxva-uc.a.run.app`).*
